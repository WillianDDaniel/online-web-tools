...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function highlightDifferences() {
    ...
    // Aplicar comparação com visualização lado a lado
    const originalA = textA.value;
    const originalB = textB.value;
    
    // Primeiro comparamos normalmente para mostrar o resultado
    performComparison();
    
    // Em seguida, forçamos a visualização lado a lado
  ...
  function performComparison(highlightMode = false) {
    ...
    // Processar os textos de acordo com as opções
    ...
    if (processedA === processedB) {
      ...
      // Animação da barra de similaridade
  ...
  function showDifferences(originalA, originalB, processedA, processedB, highlightMode) {
    ...
    // Se estamos no modo de destaque, destacamos as diferenças nos editores
    if (highlightMode) {
      highlightTextDifferences(originalA, originalB);
      return;
    }
    
    // Encontrar segmentos diferentes (simplificado)
    const segments = findDifferentSegments(originalA, originalB, processedA, processedB);
    
    // Mostrar no máximo 5 diferenças principais
    ...
    // Mostrar também a visualização lado a lado quando houver diferenças
  ...
  function findDifferentSegments(originalA, originalB, processedA, processedB) {
    // Implementação simplificada de diferenças por palavras
    const wordsA = originalA.split(/\s+/);
    const wordsB = originalB.split(/\s+/);
    
    const differences = [];
    
    // Comparação simplificada (pode ser melhorada com algoritmos mais avançados)
    let i = 0, j = 0;
    while (i < wordsA.length || j < wordsB.length) {
      ...
      if (wordA === wordB) {
        ...
        // Verificar palavras próximas (janela de busca limitada)
        const windowSize = 5;
  ...
  function highlightTextDifferences(textA, textB) {
    ...
    // Dividir os textos em linhas para comparação linha por linha
    const linesA = textA.split('\n');
    const linesB = textB.split('\n');
    
    // Criar elemento para mostrar a visualização lado a lado
    ...
    // Criar os contêineres de texto lado a lado
    ...
    // Encontrar o número máximo de linhas para comparação
    ...
    // Adicionar o contêiner de diferenças à seção
    differencesList.appendChild(diffContainer);
    
    // Adicionar título específico para a visualização
    ...
    // Adicionar classe para destacar visualmente os campos de texto
